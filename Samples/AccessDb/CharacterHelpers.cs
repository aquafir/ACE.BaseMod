...
internal static class CharacterHelpers
{
    public static Character CopyCharacter(this Character original, uint accountId, string name) =>
        CopyCharacterAs(original, accountId, name, PlayerSaveOptions.Default);

    /// <summary>
    /// Create a copy of a Character with a give name and account ID and update chosen properties with Character's GUID
    /// </summary>
    public static Character CopyCharacterAs(this Character original, uint accountId, string name, PlayerSaveOptions options)
    {
